      ...
      {product && (
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {/* Description */}
            <div>
              <p className="font-mono text-[10px] tracking-widest uppercase text-stone-400 mb-2">Description</p>
            ...
            {specs.length > 0 && (
              <div>
                <p className="font-mono text-[10px] tracking-widest uppercase text-stone-400 mb-3">General Specifications</p>
                <div className="border border-stone-200 rounded-lg overflow-hidden">
                  <table className="w-full text-sm text-left">
                    <tbody className="divide-y divide-stone-100">
                      {specs.map(([k, v]) => (
                        <tr key={k} className="hover:bg-stone-50 transition-colors">
                          <td className="px-4 py-2.5 text-stone-400 font-mono text-xs w-2/5">{k}</td>
            ...
            {variants.length > 0 && (
              <div>
                <p className="font-mono text-[10px] tracking-widest uppercase text-stone-400 mb-3">Available Variants</p>
                <div className="border border-stone-200 rounded-lg overflow-hidden">
                  <table className="w-full text-sm text-left">
                    <thead className="bg-stone-50 border-b border-stone-200">
                      <tr>
                        <th className="px-4 py-2.5 font-mono text-[10px] uppercase tracking-widest text-stone-400">#</th>
                        <th className="px-4 py-2.5 font-mono text-[10px] uppercase tracking-widest text-stone-400">Variant</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-stone-100">
                      {variants.map((v, i) => (
                        <tr key={i} className="hover:bg-stone-50 transition-colors">
                          <td className="px-4 py-2.5 text-stone-400 font-mono text-xs">{String(i + 1).padStart(2, '0')}</td>
          ...
          <div className="px-6 py-4 border-t border-stone-100 bg-stone-50 flex-shrink-0">
            <Link
              to="/contact"
              className="w-full inline-flex items-center justify-center gap-2 px-5 py-3 bg-copper-500 hover:bg-copper-600 text-white text-sm font-semibold rounded-md transition-colors"
            >
